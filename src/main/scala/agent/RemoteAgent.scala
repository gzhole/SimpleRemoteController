...
class RemoteAgentActor extends Actor {
  ...
  def receive = {
    ...
    case UpdateBinaryFile(startLocation, libName, binaryFiles) => {
      ...
      orgSender = sender
         val destuploadFolder = new File(files);
      //prepare the dest file folder
      if (!destuploadFolder.exists) {
    	  destuploadFolder.mkdirs
      }
      for {
    ...
    case Result(location) => location match {
      case null => {
        orgSender ! cmdResult("(" + this.self.path.name + ": cannot find the file " + ") --" + "Remote IP: " + Utility.getCurrentIP) //cmdResult("Done!!!")
    ...
    //  if (location != null && location.size > 0) {
        ...
        if (currentMatchClassFile.size == 0) { orgSender ! cmdResult("(" + this.self.path.name + ": cannot find the class files " + ") --" + "Remote IP: " + Utility.getCurrentIP) }
        else {
          ...
          //backup
          UnzipUtil.copyFile(file.getPath(), file.getPath() + ".gbak")
